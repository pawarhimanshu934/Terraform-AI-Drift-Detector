...
class AWSProvider(CloudProvider):
    ...
    def fetch_resources(self, expected_resources: list[ResourceModel]) -> list[ResourceModel]:
        ...
        if s3_types & set(expected_by_type):
            resources.extend(self._fetch_s3_buckets(expected_resources))
    ...
    def _fetch_s3_buckets(self, expected_resources: list[ResourceModel] | None = None) -> list[ResourceModel]:
        client = self._client("s3")
        resources: list[ResourceModel] = []
        expected_types = {r.type for r in (expected_resources or [])}
        sub_types = {
            "aws_s3_bucket_acl",
            "aws_s3_bucket_ownership_controls",
            "aws_s3_bucket_public_access_block",
        ...
        for bucket in client.list_buckets().get("Buckets", []):
            ...
            for exp in expected_resources or []:
                if exp.type in sub_types:
                    exp_bucket = exp.attributes.get("bucket") or exp.id.split(",")[0]
                    if exp_bucket == name:
                        sub_attrs = self._fetch_s3_sub_resource_attrs(client, name, exp.type, exp.id)
                        resources.append(ResourceModel(provider="aws", type=exp.type, id=exp.id, name=name, attributes=sub_attrs, tags={}, raw=bucket))

        return resources

    def _fetch_s3_sub_resource_attrs(self, client: Any, bucket_name: str, resource_type: str, exp_id: str) -> dict[str, Any]:
        if resource_type == "aws_s3_bucket_versioning":
            try:
                v_resp = client.get_bucket_versioning(Bucket=bucket_name)
                status = v_resp.get("Status", "Disabled")
                mfa_delete = v_resp.get("MFADelete", "Disabled")
                return {"id": exp_id, "bucket": bucket_name, "versioning_configuration": [{"status": status, "mfa_delete": mfa_delete}]}
            except Exception:
                return {"id": exp_id, "bucket": bucket_name}

        if resource_type == "aws_s3_bucket_public_access_block":
            try:
                pab_resp = client.get_public_access_block(Bucket=bucket_name).get("PublicAccessBlockConfiguration", {})
                return {
                    "id": exp_id,
                    "bucket": bucket_name,
                    "block_public_acls": pab_resp.get("BlockPublicAcls", False),
                    "block_public_policy": pab_resp.get("BlockPublicPolicy", False),
                    "ignore_public_acls": pab_resp.get("IgnorePublicAcls", False),
                    "restrict_public_buckets": pab_resp.get("RestrictPublicBuckets", False),
                }
            except Exception:
                return {
                    "id": exp_id,
        ...
        if resource_type == "aws_s3_bucket_ownership_controls":
            try:
                oc_resp = client.get_bucket_ownership_controls(Bucket=bucket_name)
                rules = oc_resp.get("OwnershipControls", {}).get("Rules", [])
                object_ownership = rules[0].get("ObjectOwnership") if rules else "BucketOwnerPreferred"
                return {"id": exp_id, "bucket": bucket_name, "rule": [{"object_ownership": object_ownership}]}
            except Exception:
                return {"id": exp_id, "bucket": bucket_name}

        if resource_type == "aws_s3_bucket_acl":
            acl_val = exp_id.split(",")[-1] if "," in exp_id else "private"
            return {"id": exp_id, "bucket": bucket_name, "acl": acl_val}
